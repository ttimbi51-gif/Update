...
This project uses Express for the admin API, uploads and application email. Deploy it to **Azure App Service (Node.js)**, not GitHub Pages or Azure Static Web Apps alone. Static hosting can display the HTML and committed images, but it cannot run `server.js`, save uploads, or send applications by email.

In Azure App Service:

1. Open **Deployment Center**, choose **GitHub**, select this repository and its production branch, and save the deployment.
2. Set the runtime to Node.js 20 LTS and the startup command to `npm start`.
3. Add the environment variables listed below under **Configuration > Application settings**.
4. Set `SITE_ROOT` to the public HTTPS URL of the App Service or custom domain, for example `https://www.example.com`. This enables browser viewing of uploaded PPT/PPTX files.
5. After each push to the selected GitHub branch, Azure will build and redeploy the current folders, including `images`, `billbord`, `slides`, `data` and `applications`.

Azure App Service storage is writable, but uploaded files are runtime data. Keep regular backups of `data`, `billbord`, `slides` and `applications`; a new deployment should not be treated as a backup.
...
PORT=3000
HOST=0.0.0.0
SITE_ROOT=https://your-public-domain.example
# Admin credentials are configured in Azure App Service settings.
# Do not place real passwords in this file or commit them to GitHub.
ADMIN_USERNAME=TSHIRELETSO
ADMIN_PASSWORD=SET_IN_AZURE_APP_SETTINGS
SMTP_HOST=
SMTP_PORT=587
SMTP_USER=
SMTP_PASS=
SENDER_EMAIL=
RECEIVER_EMAIL=info@waterkloofhillsecschool.co.za
EMAIL_SUBJECT=New Admissions Application
```

The application email is sent to `RECEIVER_EMAIL` and to the parent/guardian email entered on the form. `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER` and `SMTP_PASS` must be configured in Azure for delivery to work. Do not commit `.env` or real passwords to GitHub.
