...
async function loadBillboardSlides() {
  try {
    ...
    if (Array.isArray(sharedSlides) && sharedSlides.length > 0) {
      const visualSlides = sharedSlides.filter((slide) => slide.kind !== 'audio');
      slides = (visualSlides.length ? visualSlides : fallbackSlides).map((slide) => ({
        image: slide.image,
        media: slide.media || slide.image,
        kind: slide.kind || 'image',
        viewerUrl: slide.viewerUrl || '',
...
const slidePresentation = document.getElementById('slide-presentation');
const presentationFallback = document.getElementById('presentation-fallback');
...
function showSlide(index) {
  ...
  slideImage.hidden = slide.kind !== 'image';
  if (slideVideo) {
    slideVideo.hidden = slide.kind !== 'video';
    slideVideo.pause();
    if (slide.kind === 'video') {
      slideVideo.src = slide.media;
      slideVideo.load();
  ...
  if (slidePresentation) {
    const canOpenPresentation = slide.kind === 'presentation' && Boolean(slide.viewerUrl);
    slidePresentation.hidden = !canOpenPresentation;
    slidePresentation.src = canOpenPresentation ? slide.viewerUrl : 'about:blank';
    if (presentationFallback) {
      presentationFallback.hidden = slide.kind !== 'presentation' || canOpenPresentation;
    }
  }
  if (slide.kind === 'image') slideImage.src = slide.media;
