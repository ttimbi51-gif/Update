...
const SITE_ROOT = String(process.env.SITE_ROOT || '').replace(/\/$/, '');
const MEDIA_TYPES = {
...
const ADMIN_ACCOUNTS = {
  ...
  WHS: {
    password: process.env.WHS_PASSWORD || 'Waterkloof@2026',
    displayName: 'WHS Admin',
    role: 'web_admin',
...
const getRequestAccount = (req) => {
  const credentials = req.body || req.query || {};
  return getAdminAccount(credentials.username, credentials.password);
};
...
const requireDeveloper = (req, res) => {
  ...
  if (account.username !== 'TSHIRELETSO') {
    res.status(403).json({ error: 'Only the developer account can make advanced admin changes.' });
    return null;
  }
  return account;
};
...
app.get('/api/shared-slides', async (req, res) => {
  try {
      ...
      for (const file of files) {
        ...
        if (!seen.has(publicPath)) {
          ...
          mediaItems.push({
            ...
            kind,
            fileName: file,
            viewerUrl: kind === 'presentation' && SITE_ROOT
              ? `https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(`${SITE_ROOT}${publicPath}`)}`
              : null,
          });
        }
      }
    ...
    const informationSlides = Array.isArray(content.billboardSlides) && content.billboardSlides.length
      ? content.billboardSlides
      : [fallbackText];
    const fallbackItems = informationSlides.map(() => ({ image: fallbackImage, media: fallbackImage, kind: 'image' }));
    const slides = (mediaItems.length ? mediaItems : fallbackItems).map((item, index) => ({
      ...item,
      title: fallbackTitle,
...
app.get('/api/admin/accounts', (req, res) => {
  const accounts = Object.entries(ADMIN_ACCOUNTS).map(([username, meta]) => ({
    ...
    passwordHint: username === 'TSHIRELETSO' ? 'Tshire@letso51' : username === 'WHS' ? 'Waterkloof@2026' : 'Annele@2026',
  }));
  res.json(accounts);
});
...
app.post('/api/site-content', async (req, res) => {
  const account = requireDeveloper(req, res);
  if (!account) return;
...
const makeStaffId = (name) => String(name || 'staff-member')
  .trim()
  .toLowerCase()
  .replace(/[^a-z0-9]+/g, '-')
  .replace(/^-|-$/g, '') || `staff-${Date.now()}`;

app.post('/api/admin/staff-member', async (req, res) => {
  const account = requireDeveloper(req, res);
  if (!account) return;

  const { sectionTitle, staffMember } = req.body || {};
  const member = staffMember || {};
  if (!sectionTitle || !member.name || !member.role) {
    return res.status(400).json({ error: 'Section, name and role are required' });
  }

  try {
    const data = await readJsonFile(STAFF_TOUR_FILE, { sections: [] });
    data.sections = Array.isArray(data.sections) ? data.sections : [];
    let section = data.sections.find((entry) => entry.title === sectionTitle);
    if (!section) {
      section = { title: sectionTitle, cards: [] };
      data.sections.push(section);
    }
    section.cards = Array.isArray(section.cards) ? section.cards : [];
    const id = makeStaffId(member.name);
    const card = {
      id,
      ...
      link: `staff-profile.html?id=${id}`,
    };
    section.cards.push(card);
    await writeJsonFile(STAFF_TOUR_FILE, data);
    await appendActivityLog({ username: account.username, displayName: account.displayName, role: account.role, action: 'added-staff-member', staffName: card.name, ip: req.ip });
    res.json({ success: true, staffMember: card });
  } catch (error) {
...
app.delete('/api/admin/staff-member', async (req, res) => {
  const account = requireDeveloper(req, res);
  if (!account) return;

  const { sectionTitle, staffId, name } = req.body || {};
  if (!sectionTitle || (!staffId && !name)) {
    return res.status(400).json({ error: 'Section and staff member are required' });
  }

  try {
    const data = await readJsonFile(STAFF_TOUR_FILE, { sections: [] });
    const section = (data.sections || []).find((entry) => entry.title === sectionTitle);
    if (!section) return res.status(404).json({ error: 'Staff section not found' });
    const originalCount = section.cards.length;
    section.cards = section.cards.filter((card) => !((staffId && (card.id || makeStaffId(card.name)) === staffId) || (name && card.name === name)));
    if (section.cards.length === originalCount) return res.status(404).json({ error: 'Staff member not found' });
    await writeJsonFile(STAFF_TOUR_FILE, data);
    await appendActivityLog({ username: account.username, displayName: account.displayName, role: account.role, action: 'removed-staff-member', staffName: name || staffId, ip: req.ip });
    res.json({ success: true });
  } catch (error) {
...
app.put('/api/admin/staff-member', async (req, res) => {
  const account = requireDeveloper(req, res);
  if (!account) return;

  const { sectionTitle, staffId, updates } = req.body || {};
  if (!sectionTitle || !staffId || !updates) {
    return res.status(400).json({ error: 'Section, staff member and updates are required' });
  }

  try {
    const data = await readJsonFile(STAFF_TOUR_FILE, { sections: [] });
    const section = (data.sections || []).find((entry) => entry.title === sectionTitle);
    const card = section?.cards?.find((entry) => (entry.id || makeStaffId(entry.name)) === staffId);
    if (!card) return res.status(404).json({ error: 'Staff member not found' });
    ['name', 'role', 'description', 'image'].forEach((field) => {
      if (typeof updates[field] === 'string') card[field] = updates[field].trim();
    });
    card.id = card.id || makeStaffId(card.name);
    card.link = `staff-profile.html?id=${card.id}`;
    await writeJsonFile(STAFF_TOUR_FILE, data);
    await appendActivityLog({ username: account.username, displayName: account.displayName, role: account.role, action: 'updated-staff-member', staffName: card.name, ip: req.ip });
    res.json({ success: true, staffMember: card });
  } catch (error) {
...
const createApplicationReference = () => `WHSS${new Date().getFullYear()}${Date.now().toString().slice(-8)}${Math.floor(Math.random() * 90 + 10)}`;

const buildApplicationPdf = (fields, reference = '', documentNames = []) => {
  return new Promise((resolve, reject) => {
    ...
    const logoPaths = [path.join(__dirname, 'images', 'logo.jpg'), path.join(__dirname, 'images', 'logo.png')];
    ...
    doc.moveDown(0.2);
    doc.fontSize(11).text('Tel: 014 008 7315 / 076 809 0560 · Email: info@waterkloofhillsecschool.co.za', { align: logoPlaced ? 'left' : 'center' });
    ...
    doc.moveDown(0.5);
    if (reference) {
      doc.font('Helvetica-Bold').fontSize(11).fillColor('#061b3a').text(`Application reference: ${reference}`, { align: 'center' });
      doc.moveDown(0.5);
    }
    ...
    if (documentNames.length) {
      doc.font('Helvetica-Bold').fontSize(10).text('Supporting documents submitted', leftX, y);
      doc.font('Helvetica').fontSize(9).text(documentNames.map((name) => `- ${name}`).join('\n'), leftX, y + 16, { width: 425 });
    }
...
const submitApplication = async (req, res) => {
  try {
    ...
    const documentNames = (req.files || []).map((file) => file.originalname);
    const pdfBuffer = await buildApplicationPdf(fields, reference, documentNames);
    const attachments = [
      {
        filename: `${reference}-admissions-application.pdf`,
        content: pdfBuffer,
    ...
    const emailTo = process.env.RECEIVER_EMAIL || 'info@waterkloofhillsecschool.co.za';
    ...
    res.redirect(`/admissions.html?success=1&reference=${encodeURIComponent(reference)}`);
  } catch (error) {
...
app.post('/api/create-application', upload.any(), async (req, res) => {
  try {
    ...
    const documentNames = savedFiles.map((filePath) => path.basename(filePath));
    const pdfBuffer = await buildApplicationPdf(fields, reference, documentNames);
